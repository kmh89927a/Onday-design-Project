# 개발 간트 차트 명세서 (Gantt Chart) — v1.1

> **Document ID:** GANTT-001 Rev 1.1
...
| **추정 총 소요** | **약 36 영업일** (1인 직렬 기준, 복잡도 L=1d / M=2d / H=3d 가정) |
| **병렬 진행 시 단축 가능** | **약 18~20 영업일** (시나리오 B 2~3인 분업 기준) |
...
```mermaid
gantt
    title OnDay(온데이) 개발 간트 차트 v1.1 — 병렬 트랙 기반
    ...
    CMD-DIAG-004 진단 결과 저장             :diag4, after diag3 api2, 2d
...
gantt
    ...
    section ★ Critical Path
    Infra→DB→Diag→Share→Test                :crit, cp, 2026-05-18, 36d
...
| 10 | CMD-DIAG-004 | 2 | 25 | 진단 결과 저장 *(v1.1: DIAG-003 직렬 의존)* |
...
**병렬 가속 시: 약 18~20 영업일까지 단축 가능 (시나리오 B 2~3인 분업 기준)**
...
| v1.1 | 2026-05-14 | **CMD-DIAG-004 의존성 직렬 통일, 일정 숫자 일관성 통일, TEST-009 누락 사유 명시.** ① CMD-DIAG-004 의존성을 Wave 가이드 기준(직렬)으로 통일(`after diag2 api2` → `after diag3 api2`). 원본 태스크 표(line 200)상 선행 표기에서 CMD-DIAG-003가 누락된 것으로 판단 — 점수 산출 후 저장이 의미적으로 자연스러움. ② 메타 정보 "추정 총 소요" 30 → 36 영업일, "병렬 단축" 18 → 18~20 영업일(시나리오 B 기준)로 Critical Path 합과 일치시킴. ③ 요약 차트의 critical path 막대 35d → 36d. ④ TEST-009 누락 사유(결제 도메인 제거) 하단 주석 추가. |
...
>
> **📝 TEST-009 관련 주석:** TEST-009는 SRS Rev 1.6 결제 도메인 제거에 따라 `06_TASK_LIST_v1.2`(2026-04-18)에서 삭제됨 (의도된 누락). 본 간트의 Test 섹션은 TEST-001~008, TEST-010 총 9개로 구성되며 TEST-009 ID는 영구 결번 처리됨.
